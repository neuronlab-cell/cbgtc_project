...
def network_step(state, config, t_ms):
    """Advances the entire network state by dt_ms."""
    dt = config['dt_ms']
    ...
    observables = {
        'V_stn': new_stn['V'], 'V_gpe': new_gpe['V'], 'V_gpi': new_gpi['V'],
        'spikes_stn': s_stn, 'spikes_gpe': s_gpe, 'spikes_gpi': s_gpi
    }
