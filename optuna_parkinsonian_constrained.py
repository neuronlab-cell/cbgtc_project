"""
Parkinsonian State Optimization - Constrained for Beta

Key constraints based on PD biology:
- Low I_gpe (dopamine depletion reduces GPe drive)
- Weak g_gpe_stn (disinhibited STN)
- This allows STN-GPe oscillations to emerge
...
print("=" * 60)
print("PARKINSONIAN STATE - CONSTRAINED FOR BETA")
print("=" * 60)
...
def get_simulator(weight_multipliers):
    ...
    if cache_key not in simulator_cache:
        ...
        dummy = {'ISTN': 80.0, 'I_gpe': 200.0, 'I_gpi': 400.0,
        ...
        print(f"  [Compiled simulator, cache size: {len(simulator_cache)}]")
...
print("  I_gpe: 80-250 pA (low, simulating dopamine depletion)")
print("  g_gpe_stn_mult: 0.1-1.2 (weak, allowing STN disinhibition)")
...
def save_study(study, filename='results/parkinsonian_constrained_study.pkl'):
...
def objective(trial):
    # Intrinsic currents - CONSTRAINED
    intrinsic_params = {
        'ISTN': trial.suggest_float('ISTN', 80.0, 180.0),      # Higher STN drive
        'I_gpe': trial.suggest_float('I_gpe', 80.0, 250.0),    # LOW - dopamine depletion
        'I_gpi': trial.suggest_float('I_gpi', 250.0, 500.0),   # Maintain GPi output
    }
    
    # Synaptic weights - CONSTRAINED for PD
    weight_multipliers = {
        'g_stn_gpe': trial.suggest_float('g_stn_gpe_mult', 0.5, 2.5),   # STN→GPe 
        'g_gpe_stn': trial.suggest_float('g_gpe_stn_mult', 0.1, 1.2),   # WEAK - disinhibited STN
        'g_stn_gpi': trial.suggest_float('g_stn_gpi_mult', 0.5, 2.5),   # STN→GPi
        'g_gpe_gpi': trial.suggest_float('g_gpe_gpi_mult', 0.3, 2.0),   # GPe→GPi
    }
    
    # Noise - allow higher for irregularity
    noise_params = {
        'noise_stn_sigma': trial.suggest_float('noise_stn_sigma', 0.5, 4.0),
    ...
    # 3. Beta penalty - STRONGER (200x)
    beta_penalty = 0.0
    if beta_stn < BETA_MIN['stn']:
        beta_penalty += 200.0 * (BETA_MIN['stn'] - beta_stn)
    if beta_gpe < BETA_MIN['gpe']:
        beta_penalty += 200.0 * (BETA_MIN['gpe'] - beta_gpe)
    if beta_gpi < BETA_MIN['gpi']:
        beta_penalty += 200.0 * (BETA_MIN['gpi'] - beta_gpi)
    
    # 4. Beta bonus (bounded)
    beta_bonus = 0.0
    if beta_stn >= BETA_MIN['stn']:
...
best_beta_seen = [0.0]  # Track best beta found

def callback(study, trial):
    if trial.value is not None and trial.value != float('inf'):
        beta_stn = trial.user_attrs.get('beta_stn', 0)
        beta_gpe = trial.user_attrs.get('beta_gpe', 0)
        beta_gpi = trial.user_attrs.get('beta_gpi', 0)
        
        # Track best beta
        max_beta = max(beta_stn, beta_gpe, beta_gpi)
        if max_beta > best_beta_seen[0]:
            best_beta_seen[0] = max_beta
            print(f"  *** NEW BEST BETA: {max_beta*100:.1f}% ***")
        
        print(f"Trial {trial.number}: score={trial.value:.1f}, "
              f"STN={trial.user_attrs.get('r_stn',0):.1f}Hz/β{beta_stn*100:.0f}%, "
              f"GPe={trial.user_attrs.get('r_gpe',0):.1f}Hz/β{beta_gpe*100:.0f}%, "
              f"GPi={trial.user_attrs.get('r_gpi',0):.1f}Hz/β{beta_gpi*100:.0f}%")
...
print("\n" + "=" * 60)
print("STARTING CONSTRAINED PD OPTIMIZATION (1000 trials)")
print("=" * 60 + "\n")
...
print("\n" + "=" * 60)
print("PARKINSONIAN RESULTS (CONSTRAINED)")
print("=" * 60)
...
print("\n✓ Saved to results/parkinsonian_constrained_study.pkl")
print("=" * 60)
